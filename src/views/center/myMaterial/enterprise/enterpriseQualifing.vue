<template>
  <div class="box">
    ...
    <el-form ref="form" :model="form" :rules="rules" class="form" label-width="120px">
      ...
      <div class="check">
        <el-checkbox @change="checkboxChange" v-model="form.checked"></el-checkbox>&nbsp;&nbsp;同意并遵守，
        <span @click="toDigitalAgreement" style="color: #CAA14F">《数字证书服务协议》</span>
      </div>

      <el-form-item>
        <!-- <div class="enter" :class="{gray: !isCheck, yellow: isCheck}" @click="enter('form')">提交认证申请</div> -->
...
export default {
  data() {
    ...
    return {
      ...
      rules: {
        ...
        businessLicense: [{ trigger: "change", validator: validBusiness }]
      }
    };
  },
  computed: {
  ...
  created() {
    ...
    this.actionUrl =
      this.$store.state.states.baseUrl + "/greenland/resources/upload";
    this.getEchoInfo();
  },
  ...
  methods: {
    ...
    enter(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          axios
            .companyCertification(this.form)
            .then(res => {
              console.log(res);
              if (res.code === 200) {
                //status 改成1，
                ...
                this.$router.replace({
                  path: `/redirect/enterpriseQualfingStatusForSuccess`
                });
              } else {
